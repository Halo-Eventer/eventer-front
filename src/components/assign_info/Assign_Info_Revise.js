...
function Assign_Info_Revise({List,parentCategory}) {

  const [thumbnail,setThumbNail] = useState(List.thumbnail);
  const [images, setImages] = useState(List.images);
  const [cntImageInput, setCntImageInput] = useState(
    Array.from({length: List.images.length}, (item, index) => index));

  const [info, setInfo] = useState(
    {
      title: List.title,
      subtitle: List.subtitle,
      simpleExplanation: List.simpleExplanation,
...
//     "type": "주점"
// }
export default Assign_Info_Revise;
