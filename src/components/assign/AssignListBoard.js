...
function AssignListBoard({ upText }) {
  ...
  useEffect(() => {
    if (category === 'notice') setTmpList([...noticeList, ...eventList]);
    else {
      let tmp_up = [];
  ...
  useEffect(() => {
    // 공지사항/이벤트 인 경우 upList를 배너 순서에 맞게 다시 정렬
    let tmp_sorted = [];
        ...
        {boardList?.map((item, index) => (
          ...
          >
            <Flex style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
              ...
              {category === 'notice' && (
                <p>{item.time.slice(0, 10) + ' ' + item.time.slice(11, 19)}</p>
              )}
            </Flex>
            <BtnDiv>
              {category == 'notice' && !item.picked && (
                <h3
                  id={item.id}
